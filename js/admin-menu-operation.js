$(document).ready(function () {
  ...
  function initUI() {
    ...
    configUIBlocks(currentUserRole);
  }
  ...
  function configUIBlocks(currentUserRole) {
  ...
  function setConfirmBox(status) {
    if (status == CONSTANTS.MENU.CONFIRMATION.STATUS.CONFIRMED) {
      replaceClass($(".menu-confirm"), "alert-warning", "alert-success");
      hideElement($(".confirm-alert-icon"));
      unhideElement($(".confirm-success-icon"));
      $(".confirm-text").text(dateType == CONSTANTS.DATE.TODAY ? "今日点餐已上报" : "明日点餐已上报");
      replaceClass($(".table-card"), "border-warning", "border-success");
      replaceClass($(".btn-confirm"), "btn-primary", "btn-success");
      $(".btn-confirm").text("已上报");
  ...
  function loadTables() {
    fetchGroupUserInfo(currentUserRole, currentUserGroup).done(function (response) {
      checkMenuConfirmation(targetDate, currentUserGroup, function (confirmStatus) {
        /* Step 01:
  ...
  function setData(userList, currentGroup, orderStatus) {
    if (userList.length > 0) {
      for (let i = 0; i < userList.length; i++) {
        ...
        $(groupContainerClass).append("<tr class='" + userClassName + "' id='user-" + userId + "'>");
        ...
        if (orderStatus == CONSTANTS.ORDER.STATUS_USER.NOT_ORDER) {
          ...
          $(".order-status-" + userId).css({
            "color": CONSTANTS.COLOR.RED_DANGER,
            "font-weight": "bold"
          });
          ...
          checkOrderStatus(targetDate, CONSTANTS.ORDER.CHECK_TYPE.ORDER_CONTENT, userId, true).done(function (response) {
            ...
            orderCountOperation(userId, targetDate, null).done(function (orderCount) {
              ...
              if (orderNum == CONSTANTS.ORDER.CONTENT.NO_ORDER) {
                ...
                $(".order-status-" + userId).css("color", CONSTANTS.COLOR.GREEN_SUCCESS);
              }
  ...
  function setGroupStatusBar(unOrderSumArray) {
    ...
    replaceClass(statusBar, hasAllOrdered ? "alert-danger" : "alert-success", hasAllOrdered ? "alert-success" : "alert-danger");
    if (hasAllOrdered) {
      $(".divider").remove();
